...
from recipe.cope.rollout_worker import RolloutWorker
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            p_vec_lr = self.config.p_vec_lr
            prefix_params = []
            rest_params = []
            for name, param in actor_module_fsdp.named_parameters():
                if "user_prefix_embeddings" in name:
                    prefix_params.append(param)
                else:
                    rest_params.append(param)
            assert len(prefix_params) > 0, "No user_prefix_embeddings found"
            assert len(rest_params) > 0, "No rest parameters found"
            param_groups = [
                {
                    "params": rest_params, 
                    "lr": optim_config.lr,
        ...
        if rollout_name == "vllm":
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    loop.run_until_complete(rollout_sharding_manager.early_sleep())
                    log_gpu_memory_usage("After early SGLang sleep during init", logger=logger)
            except Exception as e:
                logger.error(f"Error occurred during early sleep: {e}")
    ...
    def init_model(self):
        from recipe.cope.dp_actor import DataParallelPPOActor
    ...
    def _sft_update(self, data: DataProto, zero_grad=True, step_optimizer=True):
        """
        Performs a Supervised Fine-Tuning (SFT) update.
        This is inspired by the FSDPSFTTrainer logic.
        The loss is calculated only on the tokens specified by `loss_mask`.
        """
        metrics = {}
        if zero_grad:
            self.actor.actor_optimizer.zero_grad()

        micro_batches = data.batch.split(self.config.actor.ppo_micro_batch_size_per_gpu)
        ...
        for micro_batch in micro_batches:
            ...
            with torch.autocast(device_type=device_name, dtype=torch.bfloat16):
                ...
                shift_labels = micro_batch["input_ids"][..., 1:].contiguous()
                
                loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
                loss = loss.view(shift_labels.size()) # Reshape to (batch, seq_len)

                loss_mask = micro_batch["loss_mask"][..., 1:].contiguous()
            ...
            scaled_loss = final_loss / n_micro_batches
            scaled_loss.backward()
            total_loss += scaled_loss.item()
        
        # --- Gradient Clipping and Optimizer Step ---
        # Same as in _rl_update

        grad_norm = torch.tensor(0.0)
        if step_optimizer:
            if self.config.actor.strategy == "fsdp":
                grad_norm = self.actor.actor_module.clip_grad_norm_(max_norm=1.0)
            elif self.config.actor.strategy == "fsdp2":
                from verl.utils.fsdp_utils import fsdp2_clip_grad_norm_
                grad_norm = fsdp2_clip_grad_norm_(self.actor.actor_module.parameters(), max_norm=1.0)
            else:
                raise NotImplementedError(f"not implement {self.config.actor.strategy}")

            if not torch.isfinite(grad_norm):
                if self.rank == 0:
                    print(f"WARN: SFT grad_norm is not finite: {grad_norm}, skipping optimizer step.")
                self.actor.actor_optimizer.zero_grad()
            else:
    ...
    def _rl_update(self, data: DataProto):
        """
        Performs a standard PPO Reinforcement Learning update.
        This contains the original logic of the update_actor method.
        """
    ...
    def update_actor(self, data: DataProto):
        """
        Dispatcher method for updating the actor.
        It decides whether to perform an SFT update or an RL update based on `data.meta_info`.
        """
        ...
        zero_grad = data.meta_info["zero_grad"]
        step_optimizer = data.meta_info["step_optimizer"]

        output_metrics = {}
        with self.ulysses_sharding_manager:
            if data.meta_info.get("sft_update", False):
                # --- SFT Update Path ---
                output_metrics = self._sft_update(data, zero_grad=zero_grad, step_optimizer=step_optimizer)
            else:
                # --- RL Update Path ---
                output_metrics = self._rl_update(data)

            # Common logic for both updates: LR scheduler step
            if step_optimizer:
                lrs = self.actor_lr_scheduler.get_last_lr()
                assert len(lrs) == 2, "lrs should have 2 elements"
                output_metrics["actor/lr_rest"] = lrs[0]
                output_metrics["actor/lr_prefix"] = lrs[1]
                self.actor_lr_scheduler.step()

        output = DataProto(meta_info={"metrics": output_metrics})
    ...
    def set_trainable_parameters(self, mode: str):
        if not (self._is_actor and hasattr(self, "actor_module_fsdp")):
            return

        p_vec_param_prefix = "user_prefix_embeddings"
        
        local_trainable_numel = 0
        local_total_numel = 0

        for name, param in self.actor.actor_module.named_parameters():
            is_p_vec_param = p_vec_param_prefix in name
            
            if mode == 'all':
                target_requires_grad = True
            elif mode == 'p_vec_only':
                target_requires_grad = is_p_vec_param
            elif mode == 'model_only':
                target_requires_grad = not is_p_vec_param
            else:
                raise ValueError(f"Unknown mode for set_trainable_parameters: {mode}")

            param.requires_grad = target_requires_grad

            numel = param.numel() 
            local_total_numel += numel
            if param.requires_grad:
                local_trainable_numel += numel

        stats = torch.tensor([float(local_trainable_numel), float(local_total_numel)], 
                             device=get_device_id())
        
        dist.all_reduce(stats, op=dist.ReduceOp.SUM)
        
        global_trainable_numel = stats[0].item()
        global_total_numel = stats[1].item()
        assert global_trainable_numel > 0, f"Error: No trainable parameters found in mode {mode}!"
        if self.rank == 0:
            print(f"Successfully set trainable mode to '{mode}':")
            print(f"  - Trainable params: {global_trainable_numel / 1e6:.4f} M")
            print(f"  - Total params: {global_total_numel / 1e6:.4f} M")
            print(f"  - Ratio: {(global_trainable_numel / global_total_numel * 100):.2f}%")
