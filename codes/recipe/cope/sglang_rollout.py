...
class SGLangRollout(BaseRollout):
    ...
    def _initialize_tools(self, config, processing_class):
        ...
        if hasattr(config.multi_turn, 'tool_config_path'):
            tool_config_path = config.multi_turn.tool_config_path
        else:
            tool_config_path = config.multi_turn.get('tool_config_path')
    ...
    def _initialize_interactions(self, config):
        ...
        """
        # Handle both dict and object attribute access
        if hasattr(config.multi_turn, 'interaction_config_path'):
            interaction_config_path = config.multi_turn.interaction_config_path
        else:
            interaction_config_path = config.multi_turn.get('interaction_config_path')
        
        if interaction_config_path is None:
            return {}

        interaction_config_file = interaction_config_path
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            request_sampling_params.update(
                {
                    "top_k": self.config.val_kwargs.get("top_k") if hasattr(self.config.val_kwargs, 'get') else getattr(self.config.val_kwargs, 'top_k', -1),
                    "top_p": self.config.val_kwargs.get("top_p") if hasattr(self.config.val_kwargs, 'get') else getattr(self.config.val_kwargs, 'top_p', 1.0),
                    "temperature": self.config.val_kwargs.get("temperature") if hasattr(self.config.val_kwargs, 'get') else getattr(self.config.val_kwargs, 'temperature', 1.0),
    ...
    async def _handle_engine_generate(
        ...
        print(f"max_new_tokens: {kwargs['max_new_tokens']}")
